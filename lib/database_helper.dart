...
class DataBaseHelper {
  ...
  _createMyTable(Database db, int version) async {
    await db.execute(
        "CREATE TABLE $TABLE ($ID INTEGER PRIMARY KEY, $QUOTEID INTEGER, $QUOTETEXT TEXT, $QUOTEAUTHOR TEXT)");
  }
  ...
  Future<List<Quote>> fetchSavedQuotes() async {
    var dbClient = await fetchMyDatabase;
    List<Map> maps =
    await dbClient.query(TABLE, columns: [ID, QUOTEID, QUOTETEXT, QUOTEAUTHOR]);
    List<Quote> quotes = [];
    if (maps.length > 0) {
